...
TEST(Vector, iterator_element_access) {
  ...
  std::vector<char> std_vector{'p', 'e', 'r', 'f', 'o',
                               'r', 'a', 't', 'o', 'r'};
  s21::vector<char>::iterator S21_it;
...
TEST(Vector, size_testing) {
  Test_vector logger;
  EXPECT_EQ(logger.empty_Vector.size(), (unsigned long)0);
  EXPECT_EQ(logger.five_Vector.size(), (unsigned long)5);
}
...
//   EXPECT_EQ(logger.double_Vector.max_size(),
//   logger.origin_double.max_size());
// }
...
//   logger.origin_five_vector.push_back(3);
//   EXPECT_EQ(logger.five_Vector.capacity(),
//             logger.origin_five_vector.capacity());
...
//   TestList tester;
//   EXPECT_EQ(tester.S21_list_empty.max_size(),
...
//   s21::list<int> S21_list_merge1{7, 12589, 43025, -23, 3285, 745};
//   std::list<int> std_list_merge1{7, 12589, 43025, -23, 3285, 745};
//   S21_list_merge1.merge(S21_list_merge);
//   std_list_merge1.merge(std_list_merge);
//   s21::list<int>::iterator S21_it = S21_list_merge1.begin();
//   std::list<int>::iterator std_it = std_list_merge1.begin();
//   while (S21_it != S21_list_merge1.end()) {
//     EXPECT_EQ(*S21_it, *std_it);
//     ++S21_it;
//     ++std_it;
//   }
//   EXPECT_EQ(S21_list_merge1.size(), std_list_merge1.size());
//   EXPECT_EQ(S21_list_merge1.empty(), std_list_merge1.empty());
// //   S21_it = tester.S21_lesser.begin();
// //   std_it = tester.std_lesser.begin();
// //   while (S21_it != tester.S21_lesser.end()) {
// //     EXPECT_EQ(*S21_it, *std_it);
...
TEST(List, function_merge_sorted) {
  ...
  s21::list<int> S21_list_merge{77, 125890, 430235, -236, 32858, 7457};
  std::list<int> std_list_merge{77, 125890, 430235, -236, 32858, 7457};
  s21::list<int> S21_list_merge1{7, 12589, 43023, -23, 3285, 745};
  std::list<int> std_list_merge1{7, 12589, 43023, -23, 3285, 745};
  S21_list_merge.sort();
  std_list_merge.sort();
  S21_list_merge1.sort();
  std_list_merge1.sort();
  S21_list_merge1.merge(S21_list_merge);
  std_list_merge1.merge(std_list_merge);
  s21::list<int>::iterator S21_it = S21_list_merge1.begin();
  std::list<int>::iterator std_it = std_list_merge1.begin();
  while (std_it != std_list_merge1.end()) {
    EXPECT_EQ(*S21_it, *std_it);
    ++S21_it;
    ++std_it;
  }
...
TEST(List, function_splice) {
  ...
  while (S21_it != S21_list_splice.cend()) {
    EXPECT_EQ(*S21_it, *std_it);
    ++S21_it;
    ++std_it;
  }
  ...
  while (S21_it != tester.S21_lesser.cend()) {
    EXPECT_EQ(*S21_it, *std_it);
    ++S21_it;
    ++std_it;
  }
...
//   std::set<int> or_set_empty{};
//   s21::set<int>::iterator i = set_empty.begin();
//   std::set<int>::iterator j = or_set_empty.begin();
//   EXPECT_EQ(*i, *j);
// }
...
//     s21::set<int> set_empty{};
//   std::set<int> or_set_empty{};
//   s21::set<int>::const_iterator i = set_empty.begin();
//   std::set<int>::const_iterator j = or_set_empty.begin();
//   EXPECT_EQ(*i, *j);
// }
...
TEST(Multiset, function_count) {
  s21::multiset<int> s1({1, 3, 4, 2, 2, 1});
  std::multiset<int> s2({1, 3, 4, 2, 2, 1});
  s2.insert(2);
  s1.insert(2);
  s2.erase(s2.begin());
  s1.erase(s1.begin());
...
TEST(Multiset, function_lower_bound) {
  s21::multiset<int> s1({1, 3, 4, 2});
  std::multiset<int> s2({1, 3, 4, 2});
...
//   auto it1 = s1.emplace(std::pair<int, int>(5, 5), std::pair<int, int>(51,
//   53));
//   ASSERT_EQ(*(it1[2].first), 5);
//   ASSERT_EQ(*(it1[3].first), 53);
// }
